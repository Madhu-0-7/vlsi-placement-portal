...
// Tell Next.js to automatically re-fetch new jobs every 12 hours (43200 seconds)
...
async function getJobs(): Promise<Job[]> {
  const url = 'https://jsearch.p.rapidapi.com/search?query=VLSI%20OR%20Embedded%20fresher%20India&page=1&num_pages=1&date_posted=month';
  const options = {
    method: 'GET',
    headers: {
  ...
  try {
    ...
    // Map the chaotic API data into our clean interface
    return data.map((job: any, index: number) => {
      // Handle the timestamp properly
      const dateObj = job.job_posted_at_datetime_utc 
        ? new Date(job.job_posted_at_datetime_utc) 
        : new Date();

      return {
        id: job.job_id || String(index),
        title: job.job_title || "Hardware Engineer",
        company: job.employer_name || "Confidential Company",
        source: job.job_publisher || "Career Portal",
        postedDate: dateObj.toISOString().split('T')[0],
        postedTime: dateObj.toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute:'2-digit' }) + ' IST',
        applyLink: job.job_apply_link || job.job_google_link || "#"
      };
    });
  } catch (error) {
    console.error("Failed to fetch jobs:", error);
    return []; // Return empty array if the API goes down so the site doesn't crash
  }
}
  ...
  return (
    <main className="min-h-screen bg-[#0B1120] text-slate-300 relative overflow-hidden p-6 md:p-12 font-sans selection:bg-cyan-500/30">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        {jobs.length === 0 ? (
          <div className="text-center p-10 border border-slate-700/50 rounded-xl bg-[#111827]/80 backdrop-blur-md">
            <p className="text-cyan-400 font-mono animate-pulse">Scanning frequencies for new positions...</p>
          </div>
        ) : (
            ...
            {jobs.map((job) => (
              ...
              >
                ...
                <a
                  ...
                  className="mt-6 md:mt-0 relative inline-flex items-center justify-center px-8 py-3 text-sm font-bold text-white transition-all duration-200 bg-cyan-600 font-mono rounded-lg hover:bg-cyan-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-600 focus:ring-offset-[#0B1120] overflow-hidden flex-shrink-0 ml-0 md:ml-4"
                >
